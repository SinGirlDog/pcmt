...
class content_tag {
	 ...
	 * @param $data
	 */
	public function category_ignore_menu($data) {
		$data['parentid'] = intval($data['parentid']);
		$data['catid'] = intval($data['catid']);
		if($data['parentid'] == 0){
			$parent_id = $data['catid'];
		}
		else{
	...
	/**
	 * 栏目标签任意获取,传递ID字符串英文逗号间隔 edit by SLJ
	 * @param $data
	 */
	public function category_any($data) {
		$data_catids_arr = explode(',',$data['catids']);
		$array = array();
		$siteid = $data['siteid'] && intval($data['siteid']) ? intval($data['siteid']) : get_siteid();
		$categorys = getcache('category_content_'.$siteid,'commons');
		$site = siteinfo($siteid);
		...
		foreach ($categorys as $catid=>$cat) {
			...
			if(in_array($cat['catid'],$data_catids_arr)) {
				$array[$catid] = $cat;
				$i++;
			}
		}
